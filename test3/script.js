 ...
 *   - 口算 40 题：三种运算均衡分布（约 13 加 + 13 减 + 14 乘）
 *   - 竖式 8 题 ：两数运算，三种运算随机混合
 * ----------------------------------------------------------------
 * 约束：加法结果 ≤100，减法结果 ≥10，乘法乘数 ∈ [2,9]
 * 设计要点：纯函数 + 随机生成 + 打印兼容（先试后兜底）
 */
(function () {
  "use strict";
  ...
  /* ---------- 批量生成口算题（加减乘均衡） ---------- */
  /**
   * 生成 40 道口算题：13 加 / 13 减 / 14 乘，洗牌打散避免聚集
   */
  function generateOralSet() {
    const set = [];
    const addCount = Math.floor(ORAL_COUNT / 3);
    const subCount = Math.floor(ORAL_COUNT / 3);
    const mulCount = ORAL_COUNT - addCount - subCount;
